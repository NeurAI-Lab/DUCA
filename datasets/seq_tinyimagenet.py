...
class SequentialTinyImagenet(ContinualDataset):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = MyTinyImagenet(base_path_img() + 'TINYIMG',
                                 train=True, download=True, transform=transform)
        if self.args.validation:
            train_dataset, test_dataset = get_train_val(train_dataset,
                                                    test_transform, self.NAME)
        else:
            test_dataset = TinyImagenet(base_path_img() + 'TINYIMG',
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), transforms.Compose(SequentialTinyImagenet.TRANSFORM)])
        return transform

    @staticmethod
    def get_norm_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), transforms.Compose(SequentialTinyImagenet.TRANSFORM_NORM)])
        return transform
