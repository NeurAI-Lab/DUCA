...
class ContinualDataset:
    ...
    @staticmethod
    @abstractmethod
    def get_norm_transform() -> transforms:
        """
        Returns the transform to be used for to the current dataset.
        """
        pass
...
def store_masked_mixed_loaders(train_dataset: datasets, train_dataset_sh:datasets, test_dataset: datasets,
                    setting: ContinualDataset) -> Tuple[DataLoader, DataLoader]:
    """
    Divides the dataset into tasks.
    :param train_dataset: train dataset
    ...
    train_dataset.data = train_dataset.data[train_mask]
    train_dataset_sh.data = train_dataset_sh.data[train_mask]

    test_dataset.data = test_dataset.data[test_mask]

    train_dataset.targets = np.array(train_dataset.targets)[train_mask]
    train_dataset_sh.targets = np.array(train_dataset_sh.targets)[train_mask]
    test_dataset.targets = np.array(test_dataset.targets)[test_mask]

    train_dev_sets = torch.utils.data.ConcatDataset([train_dataset, train_dataset_sh])

    train_loader = DataLoader(train_dev_sets,
                              batch_size=setting.args.batch_size*2, shuffle=True, num_workers=4)
    test_loader = DataLoader(test_dataset,
                             batch_size=setting.args.batch_size, shuffle=False, num_workers=4)
    setting.test_loaders.append(test_loader)
    setting.train_loader = train_loader
