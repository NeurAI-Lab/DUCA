...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--stable_model_update_freq', type=float, default=1.0)
    parser.add_argument('--stable_model_alpha', type=float, default=0.9)

    # Plastic Model Parameters
    parser.add_argument('--plastic_model_update_freq', type=float, default=1.0)
    parser.add_argument('--plastic_model_alpha', type=float, default=0.9)
...
class CLSER(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            ...
            l_cons = torch.mean(self.consistency_loss(self.net(buf_inputs), ema_logits))
