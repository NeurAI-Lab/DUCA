...
def save_model(args, model, model_save_dir, task):
    lst_models = ['net', 'net2', 'ema_net1', 'ema_net2', 'ema_lin']
    for model_name in lst_models:
        if model_name == 'net':
            model_save_dir_t = os.path.join(model_save_dir, args.experiment_id)
        else:
            model_save_dir_t = os.path.join(model_save_dir, args.experiment_id + f'_{model_name}')
        if hasattr(model, model_name):
            os.makedirs(model_save_dir_t, exist_ok=True)
            save_model = getattr(model, model_name)
            torch.save(save_model, os.path.join(model_save_dir_t, '{}_{}.pth'.format(model_name, task)))