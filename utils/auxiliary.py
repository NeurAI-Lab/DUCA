...
class AuxiliaryNet():
    ...
    def get_aux_transform(self):
        ...
        elif self.args.dataset == 'rot-mnist':
            transform = [transforms.ToPILImage(),
...
def get_shape_data(inputs, args, device):
    ...
    s = np.arange(0, inputs.shape[0])
    ret_tuple = (torch.stack([transform(ee.cpu())
                        for ee in inputs[s]]).to(device),)
...
class transform_sobel_edge(object):
    def __init__(self, args, upsample_size=0, p=0.0):
